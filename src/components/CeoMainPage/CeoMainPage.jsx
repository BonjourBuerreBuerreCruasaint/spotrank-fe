...
const CeoMainPage = () => {
  ...
  const fetchPlacesFromAPI = useCallback(async () => {
    try {
      ...
      const seoulDataResponse = await fetch('/api/seouldata'); // 프록시를 사용하여 API 호출
  ...
  const fetchPopulationData = useCallback(async () => {
    try {
      ...
      const response = await fetch('/api/jinfinalpeople'); // JSON 형식으로 API 호출
      console.log('jinfinalpeople:', response);
      if (!response.ok) throw new Error('유동인구 데이터 요청 실패');
      
      const rawData = await response.json(); // JSON 형식으로 응답을 가져옴
      ...
      })).filter(item => !isNaN(item.TotalPeoPle) && !isNaN(item.latitude) && !isNaN(item.longitude)); // 유효한 데이터만 필터링
